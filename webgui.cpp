...
string plotLines(list<in*> ins, unsigned long tmin, unsigned long tmax, unsigned int x, unsigned int y, string title)
{
	...
	*/
	...
	if (fp)
	{
		...
		fprintf(fp, "# gnuplot script generated by %s build %lu\n", tcProgramName, tcBuildNr);
...
double make_header(struct mg_connection *conn, unsigned int autoRefreshTime = 0){
	...
	mg_printf(conn, "<head><title>" tcProgramName " build %lu </title>\n", tcBuildNr);
	if (autoRefreshTime)
		mg_printf(conn, "<meta http-equiv=\"refresh\" content=\"%u\">\n", autoRefreshTime);
	mg_printf(conn, "</head><a href=\"/\"><h1><font color=\"#000000\">" tcProgramName " build %lu</font></a>\n", tcBuildNr);
	if (globalControl)
