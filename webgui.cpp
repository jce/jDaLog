...
string plotLines(list<in*> ins, double tmin, double tmax, unsigned int x, unsigned int y, string title)
{
	...
	bool haveKey(ins.size() > 1); 		// "Key" is Gnuplot language for "Legend", so it uses some graph-space.
	...
	*/
	...
	if (fp)
	{
		...
		fprintf(fp, "# gnuplot script generated by tcFarmControl " GIT_SHORT_HASH " " GIT_WORKING_TREE "\n");
...
double make_header(struct mg_connection *conn, unsigned int autoRefreshTime = 0){
	...
	mg_printf(conn, "<head><title>tcFarmControl " GIT_SHORT_HASH " " GIT_WORKING_TREE "</title>\n");
	if (autoRefreshTime)
		mg_printf(conn, "<meta http-equiv=\"refresh\" content=\"%u\">\n", autoRefreshTime);
	mg_printf(conn, "</head><a href=\"/\"><h1><font color=\"#000000\">tcFarmControl " GIT_SHORT_HASH " " GIT_WORKING_TREE "</font></a>\n");
	if (globalControl)
