...
string plotLines(list<in*> ins, double tmin, double tmax, unsigned int x, unsigned int y, string title)
{
	...
	*/
	...
	if (fp)
	{
		...
		fprintf(fp, "# gnuplot script generated by tcFarmControl " GIT_SHORT_HASH_WITH_MODIFIED "\n");
...
double make_header(struct mg_connection *conn, unsigned int autoRefreshTime = 0){
	...
	mg_printf(conn, "<head><title>tcFarmControl " GIT_SHORT_HASH_WITH_MODIFIED "</title>\n");
	if (autoRefreshTime)
		mg_printf(conn, "<meta http-equiv=\"refresh\" content=\"%u\">\n", autoRefreshTime);
	mg_printf(conn, "</head><a href=\"/\"><h1><font color=\"#000000\">tcFarmControl " GIT_SHORT_HASH_WITH_MODIFIED "</font></a>\n");
	if (globalControl)
